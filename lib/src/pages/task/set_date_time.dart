...
import 'package:maps_test/src/pages/task/set_repeat_modal.dart';
...
class _SelectSetDialogState extends State<SetDateTime> {
  ...
  RepeatInterval _selectedRepeatInterval = RepeatInterval.none;
  @override
  void initState() {
    super.initState();
  ...
  void _openRepeatDialog() {
    showDialog(
      context: context,
      builder: (ctx) {
        return SetRepeatDialog(initialRepeatInterval: _selectedRepeatInterval);
      },
    ).then((result) {
      if (result != null && result is RepeatInterval) {
        setState(() {
  ...
  Widget build(BuildContext context) {
    ...
    String getRepeatValueString(RepeatInterval interval) {
      switch (interval) {
        ...
        child: Column(
          ...
          children: [
            ...
            _buildControlRow(
              icon: Icons.repeat,
              title: 'Repeat',
              value: getRepeatValueString(_selectedRepeatInterval),
              ...
              children: [
                ...
                TextButton(
                  onPressed: () {
                    Navigator.of(
                      context,
                    ).pop({'date': _selectedDate, 'time': _selectedTime});
                  },
                  child: const Text(
                    'Done',
                    style: TextStyle(color: Colors.white),
